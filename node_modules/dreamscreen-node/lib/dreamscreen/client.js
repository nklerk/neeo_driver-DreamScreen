...
Client.prototype.handleReceivedPacket = function (received, rinfo) {
    ...
    if (!light) {   //new device
        ...
        if (commandReceived == 0x0103) {
            ...
            console.log(`0x010A Updating device: ${senderIp}`);
            ...
            if (productId == 1 || productId == 2) { //Dreamscreen HD, DreamScreen 4K
                ...
                light.name = utils.subBufferString(0, 16, payload);
                light.groupName = utils.subBufferString(16, 16, payload);
                light.groupNumber = payload[32];

                light.mode = payload[33];
                light.brightness = payload[34];
                light.ambientColor = received.slice(40, 43);
                light.ambientShow = payload[62];
                light.hdmiInput = payload[73];
                light.hdmiInputName1 = utils.subBufferString(75, 16, payload);
                light.hdmiInputName2 = utils.subBufferString(91, 16, payload);
                light.hdmiInputName3 = utils.subBufferString(107, 16, payload);
                ...
                light.mode = payload[33];
                light.brightness = payload[34];
                light.ambientColor = received.slice(35, 38);
                light.ambientShow = payload[60];
            }
...
Client.prototype.send = function (message, ipAddress, callback) {
    console.log(`client.send, sending message to ${ipAddress}`);
...
Client.prototype.addMessageHandler = function (commandUpper, commandLower, callback) {
    if (typeof commandUpper !== 'number') {
        throw new TypeError('LIFX Client addMessageHandler expects commandUpper parameter to be number');
    }
    if (typeof commandLower !== 'number') {
        throw new TypeError('LIFX Client addMessageHandler expects commandLower parameter to be number');
    }
    if (typeof callback !== 'function') {
